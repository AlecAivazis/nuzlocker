...
The scraper writes an intermediate `output/manifest.json` with per-variant metadata but no SHA256 or file size — those are only known after uploading to the CDN. A deploy script (not yet written) computes them, groups variants into games, produces the final CDN manifest, and writes `Nuzlocker/bundled-manifest.json` for inclusion in the app bundle.
